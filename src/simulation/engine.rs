...
impl SimulationEngine {
        ...
        match op {
            ...
            Operation::ControlledInteraction {
                ...
                if control_idx == target_idx {
                    return Err(OnqError::InvalidOperation {
                        message:
                            "Control and target QDUs cannot be the same for controlled operation"
                                .to_string(),
                    });
                }
            ...
            Operation::RelationalLock {
                ...
                if idx1 == idx2 {
                    return Err(OnqError::InvalidOperation {
                        message: "RelationalLock requires two different QDUs.".to_string(),
                    });
                }
                ...
                let target_subspace_vector: [Complex<f64>; 4] = match lock_type {
                    LockType::BellPhiPlus => {
                        [sqrt2_inv, Complex::zero(), Complex::zero(), sqrt2_inv]
                    } // (1/sqrt(2))(|00> + |11>)
                    LockType::BellPhiMinus => {
                        [sqrt2_inv, Complex::zero(), Complex::zero(), -sqrt2_inv]
                    } // (1/sqrt(2))(|00> - |11>)
                    LockType::BellPsiPlus => {
                        [Complex::zero(), sqrt2_inv, sqrt2_inv, Complex::zero()]
                    } // (1/sqrt(2))(|01> + |10>)
                    LockType::BellPsiMinus => {
                        [Complex::zero(), sqrt2_inv, -sqrt2_inv, Complex::zero()]
                    } // (1/sqrt(2))(|01> - |10>)
                };
    ...
    fn project_onto_subspace_state(
        ...
        for row in 0..4 {
            for col in 0..4 {
                proj_matrix[row][col] =
                    target_state_normalized[row] * target_state_normalized[col].conj();
            }
        }
        ...
        for i_other in 0..(dim / 4) {
            ...
            let indices = [
                ...
                i_base | k2_mask,
                i_base | k1_mask,
                i_base | k1_mask | k2_mask,
            ];
            if indices[3] >= dim {
                return Err(OnqError::SimulationError {
                    message: format!(
                        "Internal error: Calculated index {} >= dimension {} in projection application.",
                        indices[3], dim
                    ),
                });
            }
            ...
            let psi_sub = [
                current_vector[indices[0]],
                current_vector[indices[1]],
                current_vector[indices[2]],
                current_vector[indices[3]],
            ];
        ...
        for i_other in 0..(dim / 4) {
            ...
            if indices[3] >= dim {
                continue;
            } // Should not happen if first loop worked
            let psi_sub = [
                current_vector[indices[0]],
                current_vector[indices[1]],
                current_vector[indices[2]],
            ...
            for j in 0..4 {
                check_total_overlap += target_state_normalized[j].conj() * psi_sub[j];
            }
        }

        // Check if projection resulted in a non-zero state
        if total_projected_norm_sq < 1e-12 {
            // Check if the projected state has norm ~0
    ...
    pub(crate) fn stabilize(
        ...
        for (k, _) in state_vector.iter().enumerate().take(dim) {
            ...
            if amplitude_sq > 1e-12 {
                ...
                if final_score.is_finite() && final_score > 1e-12 {
                    // Use tolerance for score as well
        ...
        let mut chosen_outcome_index: usize =
            valid_outcomes.last().map(|(idx, _)| *idx).unwrap_or(0); // Default to 0 if possible_outcomes was empty, though checked above
        ...
        if !valid_outcomes
            .iter()
            .any(|(idx, _)| *idx == chosen_outcome_index)
        {
        ...
        for target_qdu_id in targets {
            if let Some(target_idx) = self.qdu_indices.get(target_qdu_id) {
                ...
                result.record_stable_state(
                    *target_qdu_id,
                    StableState::ResolvedQuality(outcome_bit as u64),
                );
            } else {
                return Err(OnqError::ReferenceViolation {
                    message: format!(
                        "QDU {} targeted for stabilization not found in simulation context.",
                        target_qdu_id
                    ),
                });
            }
    ...
    fn get_qdu_index(&self, qdu_id: &QduId) -> Result<&usize, OnqError> {
        self.qdu_indices
            ...
            .ok_or_else(|| OnqError::ReferenceViolation {
                message: format!("QDU {} not found in simulation context", qdu_id),
            })
    }
    ...
    fn apply_single_qdu_gate(
        ...
        for i in 0..dim / 2 {
            // i represents the bit pattern for the n-1 qubits *other* than target k
            ...
            if i1_raw >= dim {
                // Only need to check i1_raw as it's the largest
                return Err(OnqError::SimulationError {
                    message: format!(
                        "Internal error: Calculated index {} >= dimension {} in apply_single_qdu_gate.",
                        i1_raw, dim
                    ),
                });
            }
    ...
    fn apply_two_qdu_gate(
        ...
        if idx1 == idx2 {
            return Err(OnqError::InvalidOperation {
                message: "Target indices for a two-QDU gate cannot be the same".to_string(),
            });
        }
        ...
        for i_other in 0..(dim / 4) {
            ...
            if indices[3] >= dim {
                // Check highest index only is sufficient
                return Err(OnqError::SimulationError {
                    message: format!(
                        "Internal error: Calculated index {} >= dimension {} in apply_two_qdu_gate.",
                        indices[3], dim
                    ),
                });
            }
